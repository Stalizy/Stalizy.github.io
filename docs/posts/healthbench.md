...
- 三个被评测模型：deepseek-v4-flash（通用）、Qwen3-235B（通用）、qwen3.5-sft-sft（在 Qwen 基座上微调）。评测按第三节的框架：GRADER_TEMPLATE + 逐条判分 + calculate_score + 分维度聚合。
...
在21道题目之前，先跑了 6 题的结果，最终分基本只差0.2分，虽然扣分点不同，如果单从最终分看是无法获得排序信息的；扩大到 21 题后，误差带从 ±0.1 缩到 ±0.05~0.07。三个模型之间的差距（0.46 vs 0.40 vs 0.37）仍然只有 2~3 个标准差。21 题能看出排序方向，但不足以当严格结论。
...
模型共性的弱点比较清楚：context_awareness 三个模型都低（0.23~0.38），主动澄清、结合用户具体情境这类行为是共同短板；高紧迫性的医疗判定场景（UC 伴发热、突发视力模糊）也无法达到HealthBench制定中的理想水平。
