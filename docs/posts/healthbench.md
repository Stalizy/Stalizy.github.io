...
在 21 道题目之前，先跑了 6 题，三个模型的最终分只差 0.008，虽然扣分点不同，如果单从最终分看是无法获得排序信息的；扩大到 21 题后，误差带从 ±0.1 缩到 ±0.05~0.07。三个模型之间的差距（0.46 vs 0.40 vs 0.37）仍然只有 2~3 个标准差。21 题能看出排序方向，但不足以当严格结论。

### 4.2 分主题与分维度结果

按主题看（每主题 n=3）：

| theme               | deepseek | qwen3.5-sft | Qwen3-235B |
| ------------------- | -------- | ----------- | ---------- |
...
| axis                        | deepseek | qwen3.5-sft | Qwen3-235B |
| --------------------------- | -------- | ----------- | ---------- |
...
这次出现 5 个负分 case（单题算出来是负数，聚合时被 clip 成 0）：qwen3.5-sft 在 communication 一个 −0.068，Qwen3-235B 和 qwen3.5-sft 在 health_data_tasks 各两个（最低 −0.381），deepseek 在 hedging 一个 −0.170。负分说明回答过多触发了负分条款的结果，如果此时查看负分的题目，能比较好的看出模型在哪道题上做了危险的事。

### 4.3 结果的初步结论

模型共性的弱点比较清楚：context_awareness 三个模型都低（0.316~0.376），主动澄清、结合用户具体情境这类行为是共同短板；高紧迫性的医疗判定场景（UC 伴发热、突发视力模糊）也无法达到HealthBench制定中的理想水平。

个体差异的信号（当然21 题是不能定下严格的结论，可以初步看看小样本集下的表现）：deepseek 在 communication、complex_responses、health_data_tasks 相对强，但 hedging 明显弱（0.283 还有个负分 case）；qwen3.5-sft 在 hedging、context_seeking 相对强，但 health_data_tasks 只有 0.222；Qwen3-235B 在 health_data_tasks 直接归零。从这些种类的分数来看，从实际业务需求出发制定合理的动作和主题，这样评估的产出能够一定程度回答业务的问题。

复现性上看，21 题中 6 道题跑过两遍，就能发现同一题的分数有波动，甚至有较大波动，例如胸片题 Qwen3-235B 从 0.143 变成 −0.381。小样本下采样随机性会进一步稀释信号，固定种子、降低温度、或者对关键题做 n_repeats 都更稳妥。

从单独的分数看，能体现不同模型回答的相对好坏吗？如果在只在题目内部比，可以说在Healthbench指定的尺子下，能够确定哪个模型兼顾到符合医生共识的要点，当然单个题目无法去识别出哪个模型更优。可靠的做法是逐层次比较，不仅仅看总分还看在不同主题不同行为下的分维度分数，从而定位不同模型的强弱。
...
2. 分维度分是刚需。只报一个总分，会丢掉模型能力画像——两个同分的模型，按主题和轴切开可能是完全不同的两种表现。因此分数是为汇报设计的，逐条判定和围绕业务的分维度定义才是为改进设计的。

3. 样本量要诚实且大，成本可控下应该是越大越好，越全面越好。实际的评测中n=6 撑不起排序，n=21，也只能看方向，觉得要下结论至少 50 题往上，并且配 meta-eval 验证裁判。
